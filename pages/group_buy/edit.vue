...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        uni.showLoading({ mask: true });
        db.collection(dbCollectionName).doc(id).field("store_id,title,category,type,price,originalPrice,specialPrice,image,images,description,notice,specs,status,create_date").get().then((res) => {
