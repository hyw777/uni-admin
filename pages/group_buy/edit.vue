...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        uni.showLoading({ mask: true });
        db.collection(dbCollectionName).doc(id).field("store_id,title,category,type,price,originalPrice,specialPrice,stock,sold,image,images,description,notice,specs,status,create_date").get().then((res) => {
          const data = res.result.data[0];
          if (data) {
            this.formData = data;
            if (data.image) this.imageValue = { url: data.image, fileID: data.image, extname: '', name: '' };
            this.imagesValue = (data.images || []).map(url => ({ url: url, fileID: url, extname: '', name: '' }));
            this.specsList = data.specs && data.specs.length ? data.specs : [];
          }
        }).catch((err) => {
          uni.showModal({
            content: err.message || '请求服务失败',
